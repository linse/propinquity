...
import ddf.minim.AudioPlayer;
  ...
  Byte 1:
  __   type (2 bits)
  ___  destination address (3 bits)
  _    free  (1 bit)
  xx   not used (2 bits)
  ...
  Byte 3:
  ____  current LED states
  ____  next LED states
  
  Byte 4:
  ____  next+1 LED states
  ____  next+2 LED states
  */
  
  //if there is nothing in the file, we want to default to 2 min of lights-on free play.
  final int FREE = 1;
 ...
 // AudioPlayer posSoundPlayer;
  ...
  public boolean isInCoopMode() { return (coop && (coopPts == 0 || getTotalPts() < coopPts*2)); }
  public boolean isCoopDone() { return (coopPts != 0) && (getTotalPts()/2) >= coopPts; }
  public boolean getLastCoopDone() { return lastCoopDone; }
  ...
  void reset() {
    ...
    //stubReading = 0;
    time = 0;
    lastStep = -parent.MAX_INT;
    running = false;
    lastUpdate = 0;
    
    //rewing song
    if (songPlayer != null) songPlayer.rewind();
  }
  ...
  void step() {
    ...
    int result;
    for (int i=0; i<players.length; i++) {
      result = players[i].processStep();
      
      /*if (!MUTE) {
        if (result == -1) {
          if (coop && (coopPts == 0 || getTotalPts() < coopPts*2))
          {
            negSoundPlayer_coop.play();
            negSoundPlayer_coop.rewind();
          }
          else {
            players[i].playNegSound(); 
          }
        }
      }*/
    }
  ...
  boolean isPaused() { return !running; }
  boolean isRunning() { return running; }
  
  int successfullyRead() { return successfullyRead; }
  int getTempo() { return tempo; }
  int getMultiplier() { return multiplier; }
  int getNumPlayers() { return players.length; }
  int getNumSteps() { return numSteps; }
  boolean isDone() { return (currentStep > numSteps); } //extra step to make sure the last one got in
  ...
  public void xmlEvent(proxml.XMLElement p_xmlElement) { 
    ...
    if (l_numPlayers < 2)
    {
    	parent.println("Error: Bad level file. We need data for 2 players.");
      successfullyRead = 0;
      return;
    }
    ...
    stepInterval = (long)(60f/tempo * multiplier * 1000);
    parent.println("Step Interval: " + stepInterval);
    ...
    for (i=0; i<l_numPlayers; i++)
    {
      ...
      if(l_numStepsEach[i] > MAX_STEPS) l_numStepsEach[i] = MAX_STEPS;
      // max out at 256
    }
    
    //Now make sure that both players have the same number of steps.
    //if not, we'll let the other player have free play.
    l_numSteps = l_numStepsEach[0]; //stays this way if there is only one player.
    if (l_numPlayers > 1)
    {
      for (i=0; i<l_numPlayers; i++)
      {
        //for now this will only go up to 2
        //but let's keep the possibility of increasing the max number of players
        if (l_numStepsEach[i] > l_numSteps) l_numSteps = l_numStepsEach[i]; 
      }
    }
    ...
    if (l_numSteps > 0) readStepsUntil = l_numSteps;    
    else readStepsUntil = DEFAULT_STEPS;
    for (i=0; i<l_numPlayers; i++)
    {
      //create player
      ...
      //init player steps
    	parent.println("Sending Config for Step Interval " + stepInterval);      
      ...
      for (int j=0; j<readStepsUntil; j++)
      {
        if (j<l_numStepsEach[i])
        {
          ...
          players[i].addStep(new Step(parent.parseBoolean(l_step.getIntAttribute("pad1")), 
        		  parent.parseBoolean(l_step.getIntAttribute("pad2")),
        		  parent.parseBoolean(l_step.getIntAttribute("pad3")),
        		  parent.parseBoolean(l_step.getIntAttribute("pad4")),
        		  parent.parseBoolean(l_step.getIntAttribute("free"))),
                                      j);
        }
        else
        {
    ...
    //load song
    songPlayer = parent.minim.loadFile(SONG_FOLDER + songFile + ".mp3", 2048);
      
    //success!
    parent.println("Successfully read the level file.");
    successfullyRead = 1;
    return;
  }
    ...
    if (data.getApiID() == xbee.SERIES1_RX16PACKET)
    {
      ...
      {  
        ...
        if (player != -1) {
          ...
          //println(proximity);
          //player.processProxReading(patch, step, touched, proximity);
      ...
      else if (packet.length == XPan.CONFIG_ACK_LENGTH && packet[0] == XPan.CONFIG_ACK_PACKET_TYPE)
      {
        ...
        parent.println("Config Ack Received in Level, Turn Length is " + myTurnLength);
      }
      else if (packet.length == XPan.VIBE_IN_PACKET_LENGTH && packet[0] == XPan.VIBE_IN_PACKET_TYPE)
      {
        if (packet[2] == 4) doPause();
      }
      else 
      {
