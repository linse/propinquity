...
public class LevelSelect implements PConstants {
  ...
  final boolean[] SEND_VIBE = { true, true }; //true = vibe, false = don't vibe
  ...
  void loadLevels() {
    ...
    for(int i = 0; i < levelFiles.length; i++) {
      ...
      while (true)
        if (loadingLevel.successfullyRead() > -1) break;
  ...
  public void xmlEvent(XMLElement p_xmlElement) { 
    ...
    if (l_numPlayers < 0)
    {
      parent.println("Error: Empty level file");
  ...
  void initTextures() {
    ...
    for(int i=0; i < pgParticle.length; i++) {
      pgParticle[i] = parent.createGraphics(imgParticle[i].width, imgParticle[i].height, parent.P2D);
    ...
    pgLevel = parent.createGraphics(imgLevelParticle.width, imgLevelParticle.height, parent.P2D);
  ...
  void initP2() {
    ...
    selected++;
    if (selected >= particles.length) selected = 0;
  }
  
  void initPlayer(int player) {
    parent.println("Initializing player " + player);
    ...
    //for proximity
    if (PROX_STUB[player]) players[player].loadProxStub(player, PROX_STUB_FILE);
    else players[player].initProxComm(XBEE_PROX_1_NI[player], XBEE_PROX_2_NI[player]);
    //for accelerometer
    if (ACCEL_STUB[player]) players[player].loadAccelStub(player, ACCEL_STUB_FILE);
    else players[player].initAccelComm(XBEE_ACCEL_NI[player]);
    //for vibration
    if (SEND_VIBE[player]) players[player].initVibeComm(XBEE_VIBE_NI[player]); 
    ...
    for(int i = 0; i < particles.length; i++) {
      particles[i] = new LevelSelectParticle(parent,
        new PVector(parent.cos(parent.TWO_PI/particles.length*i) * radius, parent.sin(parent.TWO_PI/particles.length*i) * radius, 0),
        1, pgParticle[player]);
    }
  }
  ...
  private void drawSelectPlayerHUD(int player) {
    ...
    parent.text(foundAccelPatches.size() + " acceleration patch" + (foundAccelPatches.size() > 1 ? "es" : ""), 0, 50);
    if (foundUndefPatches.size() > 0)
    	parent.text("found " + foundUndefPatches.size() + " undefined patch" + (foundUndefPatches.size() > 1 ? "es" : ""), 0, 85);
    parent.popMatrix(); 
  }
  ...
  private void drawPlayerName(int player) {
    ...
    float angle = parent.TWO_PI/playerNames.length*selected;

    parent.pushMatrix();
    parent.translate(parent.cos(angle)*(parent.height/2-parent.HUD_WIDTH+parent.HUD_OFFSET),
    		parent.sin(angle)*(parent.height/2-parent.HUD_WIDTH+parent.HUD_OFFSET));
    parent.rotate(angle+parent.PI/2);
    ...
    parent.fill(255);
    parent.noStroke();
    parent.textAlign(parent.CENTER, parent.BASELINE);
    parent.textFont(font, LEVEL_FONT_SIZE);
    String name = playerNames[selected].length() > 24 ? playerNames[selected].substring(0, 24) : playerNames[selected];
    float offset = (parent.textWidth(name)/2) / (2*parent.PI*(parent.height/2-parent.HUD_SCORE_RADIUS_OFFSET)) * parent.TWO_PI;
    parent.arctext(name, parent.height/2 - parent.HUD_SCORE_RADIUS_OFFSET, angle - offset);
    parent.popMatrix();
  }
  ...
  private void drawLevelName() {
    ...
    float angle = parent.TWO_PI/levels.size()*selected;
  ...
  public void moveLeft() {
     selected--;
        if (state == LEVEL_SELECT_P1 || state == LEVEL_SELECT_P2) {
          if (selected < 0) selected = particles.length - 1;
          if (playerNames[selected] == players[0].name) keyPressed(parent.key, parent.keyCode);
        }
        else if (state == LEVEL_SELECT_SONG) {
          if (selected < 0) selected = levels.size() - 1;
        }
  }
  ...
  public void moveRight() {
    selected++;
        if (state == LEVEL_SELECT_P1 || state == LEVEL_SELECT_P2) {
          ...
          if (selected >= levels.size()) selected = 0;
        }
  }
  ...
  public boolean isDone() { return state == LEVEL_SELECTED; }
  ...
  private String[] listFileNames(String dir, String ext) {
    ...
    if (file.isDirectory()) {
      ...
      ArrayList parsedNames = new ArrayList();
  ...
  public void xBeeEvent(XBeeReader xbee) {  
    ...
    if (buffer.length > 11) {
      ...
      String serial = "";
      for(int i = 3; i < 11; i++)
        serial += parent.hex(buffer[i], 2);
      String name = "";
      for(int i = 11; i < buffer.length; i++)
        name += parent.parseChar(buffer[i]);
      
      switch (buffer[11]) {
        case 'P':
          foundProxPatches.add(serial);
          parent.println(" Found proximity patch: " + name + " (" + serial + ")");
          break;
        case 'V':
          foundVibePatches.add(serial);
          parent.println(" Found vibration patch: " + name + " (" + serial + ")");
          break;
        case 'A':
          foundAccelPatches.add(serial);
          parent.println(" Found acceleration patch: " + name + " (" + serial + ")");
          break;
        default:
          foundUndefPatches.add(serial);
          numProxPatches++; // change this later. should really be prox patches, not undefined.
          parent.println(" Found undefined patch: " + name + " (" + serial + ")");
          break;
      }
    }
    ...
    else if (buffer.length == XPan.CONFIG_ACK_LENGTH && buffer[0] == XPan.CONFIG_ACK_PACKET_TYPE)
    {
      ...
      numConfigAcks++;
      parent.println("Config Ack Received in Level Select, Turn Length is " + myTurnLength);
    }
