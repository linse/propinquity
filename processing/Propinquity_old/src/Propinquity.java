...
final String HUD_FONT = "hud/Calibri-Bold-32.vlw";
...
PFont font;
PImage hudInnerBoundary;
PImage hudOutterBoundary;
PImage hudImgPlay;
PImage hudImgLevelComplete;
PImage hudImgPlayAgain;
...
void initLevel(Player[] players, String levelFile) {
  ...
  while (true)
    if (level.successfullyRead() > -1) break;
...
void initBoundary() {
  hudInnerBoundary = loadImage("hud/innerBoundary.png");
  hudOutterBoundary = loadImage("hud/outterBoundary.png");
}

void initHUD() {
  compSound = minim.loadFile("sounds/comp.mp3", 2048);
  compSound.setGain(5);
  hudImgPlay = loadImage("hud/sbtoplay.png");
  hudImgLevelComplete = loadImage("hud/levelcomplete.png");
  hudImgPlayAgain = loadImage("hud/sbtoplayagain.png");
  ...
  hudMask.noStroke();
  hudMask.fill(255);
  hudMask.ellipse(width/2, height/2, height-HUD_WIDTH*2+BOUNDARY_WIDTH, height-HUD_WIDTH*2+BOUNDARY_WIDTH);
  hudMask.endDraw();
}
...
void initFence() {
  Body innerFence = null;
  {
    ...
    for(int i=0; i<FENCE_SECTIONS; i++)
    {
      ...
      sd.setAsBox(perimeter/FENCE_SECTIONS, fenceDepth,
                  new Vec2(cos(angle)*radius, sin(angle)*radius),
                  angle+PI/2);
  ...
  Body outerFence = null;
  {
    ...
    float fenceDepth = 0.2f;
    float worldScale = height/WORLD_SIZE;
    float radius = (WORLD_SIZE-(HUD_WIDTH/worldScale))/2f + fenceDepth/2;
    float perimeter = 2*PI*radius;
...
void initParticles() {
  ...
  //init particles
  //particles = new Particle[level.getNumPlayers()][MAX_PARTICLES / level.getNumPlayers()];
  ...
  if (numStepsPerPeriod == 0) ++numStepsPerPeriod;
...
void drawXBeeManager() {
  ...
  textFont(font, HUD_FONT_SIZE);
  textAlign(CENTER, CENTER);
  fill(255);
  noStroke();
  text("Detecting XBee modules... ", 0, 0);
  translate(0, 30);
  textFont(font, HUD_FONT_SIZE*0.65f);
  text(msg, 0, 0);
  popMatrix();
}
...
void drawPlay() {
  drawInnerBoundary();
  if (DRAW_PARTICLES) drawParticles();
  drawMask();
  drawOuterBoundary();

  //drawOuterFence();
  if (DEBUG) drawDebugFence();
  ...
  if (level.isDone()) {
    ...
    if (endedLevel) {
      ...
      //flag as ended
      if (doneTime != -1 && frameCount > doneTime + FPS*END_LEVEL_TIME)
        endedLevel = true;
    }
...
void drawParticles() {
  ...
  for(int i=0; i < level.getNumPlayers(); i++) drawParticles(i); 
}
...
void drawParticles(int p) {
  ...
  ListIterator it = particles[p].listIterator();
  while(it.hasNext())
...
/*  if ((level.isCoop() && !level.isCoopDone()) != lastCoopDone)
    playsound
  lastCoopDone = level.isCoop() && !level.isCoopDone();*/
  
  if (level.isCoop() && !level.isCoopDone()) {
      float angle = hudAngle-PI/2;
      pushMatrix();
      translate(width/2 + cos(angle)*(height/2-HUD_WIDTH+HUD_OFFSET),
                height/2 + sin(angle)*(height/2-HUD_WIDTH+HUD_OFFSET));
      rotate(angle+PI/2);
      ...
      String name = "Coop";
      while(textWidth(score+name) < 240) name += ' ';
      
      arctext(name+score, height/2 - HUD_SCORE_RADIUS_OFFSET, angle-HUD_SCORE_ANGLE_OFFSET);
    ...
    for(int i = 0; i < level.getNumPlayers(); i++) {
      ...
      player.approachHudTo(-PI/2 + TWO_PI/level.getNumPlayers()*i);
      ...
      String name = player.getName().length() > 12 ?
        player.getName().substring(0, 12) : player.getName();
      while(textWidth(score+name) < 240) name += ' ';
...
void updateParticles() { for(int i=0; i<level.getNumPlayers(); i++) updateParticles(i); }
...
void releaseParticles(int p, int nParticles) {
  ...
  float releaseAngle = level.getTime()*HUD_SCORE_ROT_SPEED/EMITTER_ANGULAR_VELOCITY;
  if (p%2 == 1) releaseAngle += PI;
  ...
  for (int i=0; i<nParticles; ++i) {
    BodyDef bd = new BodyDef();
    bd.position = new Vec2(cos(releaseAngle)*(EMITTER_RADIUS*random(0.8f, 1)),
                           sin(releaseAngle)*(EMITTER_RADIUS*random(0.8f, 1)));
    ...
    //particles[p][i] = new Particle(b, sh, PARTICLE_SCALE*random(1.0-PARTICLE_SCALE_RANGE, 1.0), pgParticle[p]);
    particles[p].add(new Particle(this, b, sh, PARTICLE_SCALE*random(1.0f-PARTICLE_SCALE_RANGE, 1.0f), pgParticle[p]));
  }
...
void killParticles(int p, int nParticles) {
  ...
  for(int i = 0; nParticles > 0 && particles[p].size() > 0; i++) {
    particle = (Particle)particles[p].removeFirst();
    if (particle == lastPeriodParticle[p]) killedLastPeriodParticle = true;
...
void liquify() { for(int i=0; i < level.getNumPlayers(); i++) liquify(i); }
void liquify(int p) {
...
void pushPeriod() { pushPeriod(false); }
void pushPeriod(boolean override) {
  ...
  for(int p = 0; p < level.getNumPlayers(); p++) {
    ...
    //println("num steps per period: " + numStepsPerPeriod);
      ...
      if (!override && (lastPeriodStep == cStep || cStep%numStepsPerPeriod != 0)) continue;
...
void updateHUD(float targetAngle, float targetAcceleration, float maxVelocity) {
  ...
  if (hudVelocity/dir > maxVelocity) hudVelocity = dir*maxVelocity;  
...
public void keyPressed() {
  /*switch (key) {
    case 'ENTER':
      if (!mmOutput) {
        ...
        mm = new MovieMaker(this, width, height, "Propinquity.mov",
  ...
  if (gameState == STATE_XBEE_INIT) {
    ...
    switch (key) {
      ...
      case '2':
        particleRadius -= 0.01;
        if (particleRadius < 0) particleViscosity = 0;
        break;
      case '4':
        particleViscosity -= 0.001;
        if (particleViscosity < 0) particleViscosity = 0;
        break;
      case '6':
        particleViscosity += 0.001;
      case 'e': //play stub
