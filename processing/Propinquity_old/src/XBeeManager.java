...
public class XBeeManager implements Runnable {
  ...
  public void run() {
    ...
    long initLastCheck;
    parent.println("Initializing XBees...");
    ...
    for(int initPortIndex = 0; initPortIndex < initPorts.length; initPortIndex++) {
      ...
      if ((osName.indexOf("Mac") != -1) && (initPorts[initPortIndex].indexOf("tty.usbserial") == -1)) {
        parent.println(" Skipping port: " + initPorts[initPortIndex]);
        continue;
      }
      
      parent.print(" Connecting to port: " + initPorts[initPortIndex] + " ... ");
      XBeeReader xbee = new XBeeReader(parent, new Serial(parent, initPorts[initPortIndex], XBEE_BAUDRATE));
      parent.println();
      parent.println("   -starting xbee");
      xbee.startXBee();
      ...
      if (initFound) {
        parent.println(initNodeId);
        ports.put(initNodeId, initPorts[initPortIndex]);
      }
      else {
        parent.println("no XBee found");
      }

      //clean up      
      parent.println("   -stopping xbee");
      xbee.stopXBee();
      parent.println("   -waiting");
      while(xbee.isAlive()) {
  ...
  public void xBeeEvent(XBeeReader xbee) {     
    //parent.println("xbee event: xbee manager");
  ...
  public String foundPortIds() {
    ...
    String nodesString = "";
    while (it.hasNext()) nodesString += it.next() + ", ";
  ...
  public void load() {
    parent.println("Loading XBee configuration");
    ...
    for(int i = 0; i < xbeeList.length; i++) {
      ...
      if (equalIndex != -1) {
        ...
        parent.println(" Using port: " + port + " ... " + nodeId);
