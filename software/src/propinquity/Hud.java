...
public class Hud {
	...
	public void drawBanner(String text, Color color, float angle, PImage bannerImg) {
		...
		parent.translate(PApplet.cos(angle) * (parent.height/2 - WIDTH + OFFSET + 2), PApplet.sin(angle)
				* (parent.height/2 - WIDTH + OFFSET + 2));
		...
		float offset = (parent.textWidth(cropped_text)/2)/(2 * PApplet.PI * (parent.height/2 - SCORE_RADIUS_OFFSET)) * PApplet.TWO_PI;
